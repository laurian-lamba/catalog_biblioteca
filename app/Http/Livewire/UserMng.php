...
class UserMng extends Component
{
    ...
    public $form_mode = "";
    public $sel_uid = [];
    protected $listeners = ["deleteUser", "deleteThisEntry", "changeUserStatus", "deleteBulk", "confirm"];
...
//        }
    ...
    public function assign()
    {
        ...
        if ($this->sel_course && $this->sel_year) {
            if (!$this->assigned_to) {
                $this->assigned_to = array();
            }
            array_push($this->assigned_to, array($this->sel_course => $this->sel_year));
    ...
    public function printIdCards()
    {
        $ids = $this->sel_uid;
        if (is_countable($ids) && count($ids)) {
            return redirect(route('print_id_cards', ["ids" => implode(",", $ids)]));
        } else {
            $this->sendMessage(__("common.nothing_has_been_selected"));
        }
    }
    ...
    public function saveUser()
    {
        ...
        ], ["phone.min" => __("common.phone_no_min_10"),
        ...
        if ($this->mode == "create") {
            ...
            $to_update = ["name" => $this->full_name, "email" => $this->email];
            if (!empty($this->password_confirmation)) {
                $to_update["password"] = Hash::make($this->password_confirmation);
            }
            $user_obj = User::updateOrCreate(["id" => $this->edit_id], $to_update);
        }
        if ($user_obj) {
        ...
        if ($id) {
            ...
            if ($user_obj) {
                ...
                if ($user_meta_obj) {
                    ...
                    //$this->sel_course = $user_meta_obj->get_std();
